...
```powershell
godot --headless --path . -s res://tools/initialize_project.gd -- --name=MyGame --release-target=none --input-capability=all
```

Use `--release-target=itch` only after configuring the required GitHub itch/GameAnalytics values. Choose `all`, `desktop`, `desktop_gamepad`, `mobile`, `gamepad`, or `custom` for the input capability; this explicit choice prevents a generated game from shipping an accidental template default. Commit the resulting `project.godot` and `template_project.cfg` changes.

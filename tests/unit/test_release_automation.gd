...
func test_web_export_preset_is_committed_for_ci_exports() -> void:
	...
	assert_true(preset_text.contains('variant/thread_support=true'))
	assert_true(preset_text.contains('variant/extensions_support=true'))
	assert_true(preset_text.contains('window.__godotMarkReady'))
...
func test_itch_workflow_runs_tests_exports_web_and_deploys_from_main() -> void:
	...
	assert_true(workflow_text.contains("Install web smoke test tooling"))
	assert_true(workflow_text.contains("Smoke test exported web build in desktop and mobile Chromium"))
	assert_true(workflow_text.contains("tools/web_smoke_check.cjs"))
	assert_true(workflow_text.contains("npx playwright install --with-deps chromium"))

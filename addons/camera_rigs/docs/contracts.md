...
The project stores a bitmask of keyboard, mouse, gamepad, and touch capabilities. Named bundles are All, Desktop, Desktop+Gamepad, Mobile+Gamepad, Gamepad, and Custom. Old profile IDs migrate to their exact device sets. Fresh generated projects must confirm a bundle during template initialization.

G.U.I.D.E. contexts are scene-owned. Lower numeric priorities win; the camera router enables/disables only its own contexts. GUI gets first refusal because GUIDE reads unhandled input. Set edge scroll's UI/drag blocking flags while the pointer is over world-blocking UI or a selection drag is active.

Camera actions are shared mutable resources. Independently controlled split-screen cameras must each build/duplicate their action and context set or gate one active consumer. Context changes from action callbacks must be deferred. Remapping is a global UI operation and temporarily disables every GUIDE context, so camera adapters clear gesture state and tolerate reactivation.

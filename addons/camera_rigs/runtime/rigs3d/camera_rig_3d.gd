...
signal view_mode_changed(previous: CameraRigTypes.ViewMode3D, current: CameraRigTypes.ViewMode3D)
...
signal output_driver_changed(previous: CameraRigTypes.OutputDriver, current: CameraRigTypes.OutputDriver)
...
@export var preset: CameraPreset3D:
	set(value):
		preset = value
		if is_node_ready():
			_configure_preset()
...
@export var output_driver: CameraRigTypes.OutputDriver = CameraRigTypes.OutputDriver.NATIVE:
	set(value):
		var previous: CameraRigTypes.OutputDriver = output_driver
		output_driver = value
		_update_native_camera_state()
		if previous != value:
...
@onready var _motion_effects: CameraMotionEffects3D = %MotionEffects3D
...
var _view_mode: CameraRigTypes.ViewMode3D = CameraRigTypes.ViewMode3D.THIRD_PERSON
var _requested_toggle_mode: CameraRigTypes.ViewMode3D = CameraRigTypes.ViewMode3D.THIRD_PERSON
...
func _process(delta: float) -> void:
	...
	if anchor_world is Vector3 and preset.zoom_anchor != CameraRigTypes.ZoomAnchor.VIEW_CENTER:
...
func apply_preset(next_preset: CameraPreset3D) -> void:
	if next_preset == null:
		return
	preset = next_preset
...
func _configure_preset() -> void:
	...
	if is_instance_valid(_spring_arm):
		_spring_arm.collision_mask = (
			...
			if output_driver == CameraRigTypes.OutputDriver.NATIVE
			and preset.occlusion_mode != CameraRigTypes.OcclusionMode.DISABLED
			else 0
		)
...
func set_user_preferences(preferences: CameraUserPreferences) -> void:
	if preferences == null:
...
func toggle_view_mode() -> void:
	if not _input_enabled or preset == null:
		return
	if preset.view_policy != CameraRigTypes.ViewPolicy3D.TOGGLE:
		return
	_requested_toggle_mode = (
		CameraRigTypes.ViewMode3D.FIRST_PERSON
		if _requested_toggle_mode == CameraRigTypes.ViewMode3D.THIRD_PERSON
		else CameraRigTypes.ViewMode3D.THIRD_PERSON
	)
	_set_view_mode(_requested_toggle_mode)


func set_view_mode(mode: CameraRigTypes.ViewMode3D) -> void:
	_requested_toggle_mode = mode
	_set_view_mode(mode)


func get_view_mode() -> CameraRigTypes.ViewMode3D:
	return _view_mode
...
func set_output_driver(driver: CameraRigTypes.OutputDriver) -> void:
	output_driver = driver


func get_output_driver() -> CameraRigTypes.OutputDriver:
...
func set_phantom_bridge(bridge: Node) -> void:
	...
	if is_node_ready() and output_driver == CameraRigTypes.OutputDriver.PHANTOM:
		_apply_camera_state()
...
func get_motion_effects() -> CameraMotionEffects3D:
...
func _pan_world(movement: Vector3) -> void:
	...
	if is_following():
		match preset.follow_interruption:
			CameraRigTypes.FollowInterruption.HARD_LOCK:
				return
			CameraRigTypes.FollowInterruption.OFFSET_WHILE_FOLLOWING:
				_follow_offset += movement
			CameraRigTypes.FollowInterruption.BREAK_ON_PAN:
				_follow_active = false
...
func _target_position_for_mode(target: Node3D) -> Vector3:
	...
	return target_position + (
		...
		if _view_mode == CameraRigTypes.ViewMode3D.FIRST_PERSON
...
func _update_view_mode() -> void:
	...
	match preset.view_policy:
		CameraRigTypes.ViewPolicy3D.THIRD_PERSON_ONLY:
			_set_view_mode(CameraRigTypes.ViewMode3D.THIRD_PERSON)
		CameraRigTypes.ViewPolicy3D.FIRST_PERSON_ONLY:
			_set_view_mode(CameraRigTypes.ViewMode3D.FIRST_PERSON)
		CameraRigTypes.ViewPolicy3D.TOGGLE:
			_set_view_mode(_requested_toggle_mode)
		CameraRigTypes.ViewPolicy3D.ZOOM_BLEND:
			if _view_mode == CameraRigTypes.ViewMode3D.THIRD_PERSON and _zoom_target <= preset.first_person_enter_zoom:
				_set_view_mode(CameraRigTypes.ViewMode3D.FIRST_PERSON)
			elif _view_mode == CameraRigTypes.ViewMode3D.FIRST_PERSON and _zoom_target >= preset.first_person_exit_zoom:
				_set_view_mode(CameraRigTypes.ViewMode3D.THIRD_PERSON)


func _set_view_mode(mode: CameraRigTypes.ViewMode3D) -> void:
	if mode == _view_mode:
		return
	var previous: CameraRigTypes.ViewMode3D = _view_mode
	_view_mode = mode
...
func _apply_camera_state() -> void:
	...
	if preset.pitch_policy == CameraRigTypes.PitchPolicy.ZOOM_CURVE:
	...
	match preset.zoom_mechanism:
		CameraRigTypes.ZoomMechanism3D.ORTHOGRAPHIC_SIZE:
			_metric_projection = Camera3D.PROJECTION_ORTHOGONAL
		CameraRigTypes.ZoomMechanism3D.FOV:
			_metric_fov = lerpf(preset.min_fov, preset.max_fov, _zoom_normalized)
		CameraRigTypes.ZoomMechanism3D.COMPOSITE:
	...
	if output_driver == CameraRigTypes.OutputDriver.PHANTOM:
...
func _desired_distance() -> float:
	...
	match preset.zoom_mechanism:
		CameraRigTypes.ZoomMechanism3D.FOV, CameraRigTypes.ZoomMechanism3D.ORTHOGRAPHIC_SIZE:
...
func _apply_motion_effects() -> void:
	if output_driver != CameraRigTypes.OutputDriver.NATIVE or not is_instance_valid(_motion_effects):
		return
...
func _update_occlusion_search(delta: float) -> void:
	if output_driver != CameraRigTypes.OutputDriver.NATIVE:
		...
		return
	if preset.occlusion_mode != CameraRigTypes.OcclusionMode.PULL_IN_AND_SEARCH:
...
func _get_zoom_anchor_world() -> Variant:
	match preset.zoom_anchor:
		CameraRigTypes.ZoomAnchor.VIEW_CENTER:
			return null
		CameraRigTypes.ZoomAnchor.FOLLOW_TARGET:
			var target: Node3D = get_follow_target()
			return _target_position_for_mode(target) if target != null else _focus_position
		CameraRigTypes.ZoomAnchor.SELECTION_TARGET:
...
func _apply_phantom_output(desired_distance: float, applied_pitch: float) -> void:
	if not is_instance_valid(_phantom_bridge):
		_report_phantom_failure("CameraRig3D is configured for PHANTOM output but no bridge is bound.")
		return
	...
	if _view_mode == CameraRigTypes.ViewMode3D.THIRD_PERSON and _phantom_bridge.has_method(&"apply_3d_third_person"):
...
func _get_mode_anchor(mode: CameraRigTypes.ViewMode3D) -> Node3D:
	return (
		_get_weak_node(_first_person_anchor_ref)
		if mode == CameraRigTypes.ViewMode3D.FIRST_PERSON
...
func _update_native_camera_state() -> void:
	if not is_node_ready() or not is_instance_valid(_camera):
		return
	_camera.current = active and output_driver == CameraRigTypes.OutputDriver.NATIVE

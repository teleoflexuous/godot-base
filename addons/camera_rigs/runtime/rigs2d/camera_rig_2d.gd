...
@export var preset: CameraPreset2D
...
func apply_preset(next_preset: CameraPreset2D, preserve_position: bool = true) -> void:
	preset = next_preset if next_preset != null else CameraPreset2D.new()
...
func _resolve_scene_nodes() -> void:
	...
	if not is_instance_valid(_camera):
		push_error("CameraRig2D requires a Camera2D at camera_path.")
	if not is_instance_valid(_focus_pivot):
		push_error("CameraRig2D requires a Node2D at focus_pivot_path.")
	if not is_instance_valid(_effect_pivot):
		push_error("CameraRig2D requires a Node2D at effect_pivot_path.")
...
func _set_zoom_target(next_zoom: float, immediate: bool) -> void:
	...
	_zoom_anchor_active = preset.zoom_anchor != CameraRigTypes.ZoomAnchor.VIEW_CENTER
...
func _update_output_driver(previous: CameraRigTypes.OutputDriver) -> void:
	var native_output: bool = output_driver == CameraRigTypes.OutputDriver.NATIVE
	if is_instance_valid(_camera):
...
func _apply_phantom_output() -> void:
	...
	if not is_instance_valid(_phantom_bridge) or not _phantom_bridge.has_method(&"apply_2d"):
		_report_phantom_failure("CameraRig2D PHANTOM output needs a bridge exposing apply_2d().")
		return
	...
	if applied is bool and not applied:
		_report_phantom_failure("CameraRig2D Phantom bridge rejected the current configuration.")
		return

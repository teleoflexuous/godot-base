class_name CameraOccluder3D
extends Node

@export var visual_targets: Array[NodePath] = []
@export var fade_mode: CameraFadeTypes.FadeMode = CameraFadeTypes.FadeMode.AUTO
...
func validate_configuration() -> PackedStringArray:
	...
	for path: NodePath in visual_targets:
		...
		elif fade_mode == CameraFadeTypes.FadeMode.INSTANCE_SHADER_PARAMETER and not _has_shader_parameter(target):
...
func _apply_amount(amount: float) -> void:
	for path: NodePath in visual_targets:
		...
		if target == null:
			continue
		var id: int = target.get_instance_id()
		var use_shader: bool = fade_mode == CameraFadeTypes.FadeMode.INSTANCE_SHADER_PARAMETER or (
			fade_mode == CameraFadeTypes.FadeMode.AUTO and _has_shader_parameter(target)
		)
		if use_shader:

class_name CameraDetailProfile
extends Resource

@export var bands: Array[CameraDetailBand] = []
...
func get_band_index(metric: float) -> int:
	...
	for index: int in range(bands.size()):
		var band: CameraDetailBand = bands[index]
...
func get_hysteretic_band_index(metric: float, current_index: int) -> int:
	...
	if candidate_index > current_index:
		var entering_band: CameraDetailBand = bands[candidate_index]
		...
		var current_band: CameraDetailBand = bands[current_index]

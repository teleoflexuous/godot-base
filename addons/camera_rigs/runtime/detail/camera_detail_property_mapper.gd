class_name CameraDetailPropertyMapper
extends Node

@export var coordinator_path: NodePath
@export var bindings: Array[CameraDetailPropertyBinding] = []

var _coordinator: CameraDetailCoordinator
...
func _ready() -> void:
	...
	if not coordinator_path.is_empty():
		set_coordinator(get_node_or_null(coordinator_path) as CameraDetailCoordinator)
...
func set_coordinator(coordinator: CameraDetailCoordinator) -> void:
	...
	if _coordinator != null:
		...
		if result != OK:
			push_warning("CameraDetailPropertyMapper could not connect to its coordinator.")
		apply_band(_coordinator.get_current_band())


func apply_band(band_id: StringName) -> void:
	for index: int in range(bindings.size()):
		var binding: CameraDetailPropertyBinding = bindings[index]

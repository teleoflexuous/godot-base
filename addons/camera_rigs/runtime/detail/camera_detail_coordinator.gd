class_name CameraDetailCoordinator
extends Node
...
@export var profile: CameraDetailProfile
@export var metric_source: CameraRigTypes.DetailMetricSource = CameraRigTypes.DetailMetricSource.DESIRED
...
func set_source(source: Node) -> void:
	...
	if _source != null and _source.has_signal(&"view_metrics_changed"):
		...
		if result != OK:
			push_warning("CameraDetailCoordinator could not connect to view_metrics_changed.")
...
func update_from_metrics(metrics: Dictionary) -> void:
	...
	var key: StringName = (
		...
		if metric_source == CameraRigTypes.DetailMetricSource.DESIRED

class_name CameraShake2DCompat
extends Camera2D

## Camera2D-compatible wrapper for callers that only use shake(amplitude,
## duration, frequency). New rigs compose CameraMotionEffects2D on an effect
...
var _motion_effects: CameraMotionEffects2D
...
func _ensure_motion_effects() -> void:
	if is_instance_valid(_motion_effects):
		return
	_motion_effects = CameraMotionEffects2D.new()
	_motion_effects.name = "CameraMotionEffects2D"

class_name CameraGuideDefaultsBuilder
extends RefCounted

const CATEGORY: String = "Camera"


static func build() -> CameraGuideDefaults:
	var result: CameraGuideDefaults = CameraGuideDefaults.new()
	...
	result.context_set = CameraGuideContextSet.new()
...
static func _build_actions() -> CameraGuideActions:
...
static func _build_keyboard_context(actions: CameraGuideActions) -> GUIDEMappingContext:
...
static func _build_mouse_context(actions: CameraGuideActions) -> GUIDEMappingContext:
...
static func _build_gamepad_context(actions: CameraGuideActions) -> GUIDEMappingContext:
...
static func _build_touch_context(actions: CameraGuideActions) -> GUIDEMappingContext:
...
static func _scale_modifier(scale: Vector3) -> GUIDEModifierScale:
	...
	# CameraInputAdapter applies delta to rate actions and never to device deltas.

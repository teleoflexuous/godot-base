class_name CameraViewVisibility3D
extends Node
...
func set_camera_rig(camera_rig: Node) -> void:
	...
	if _camera_rig != null and _camera_rig.has_signal(&"view_mode_changed"):
		...
		if result != OK:
			push_warning("CameraViewVisibility3D could not connect to view_mode_changed.")
...
func apply_view_mode(mode: int) -> void:
	_target_amount = 1.0 if mode == CameraRigTypes.ViewMode3D.FIRST_PERSON else 0.0
	set_process(true)

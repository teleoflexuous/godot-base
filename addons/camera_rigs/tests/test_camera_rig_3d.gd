...
const RIG_SCENE: PackedScene = preload("res://addons/camera_rigs/scenes/camera_rig_3d.tscn")
...
func test_scene_uses_pivot_boom_hierarchy_and_direct_camera_child() -> void:
	var rig: CameraRig3D = await _create_rig()
...
func test_positive_zoom_steps_move_toward_close_end() -> void:
	var rig: CameraRig3D = await _create_rig()
	var local_preset: CameraPreset3D = rig.preset.duplicate(true) as CameraPreset3D
...
func test_zoom_blend_view_policy_has_hysteresis_and_two_argument_signal() -> void:
	var rig: CameraRig3D = await _create_rig()
	var local_preset: CameraPreset3D = rig.preset.duplicate(true) as CameraPreset3D
	local_preset.view_policy = CameraRigTypes.ViewPolicy3D.ZOOM_BLEND
	local_preset.first_person_enter_zoom = 0.1
	local_preset.first_person_exit_zoom = 0.2
	rig.apply_preset(local_preset)
	watch_signals(rig)
	rig.set_zoom_normalized(0.05, true)
	assert_eq(rig.get_view_mode(), CameraRigTypes.ViewMode3D.FIRST_PERSON)
	assert_signal_emitted_with_parameters(
		rig,
		"view_mode_changed",
		[CameraRigTypes.ViewMode3D.THIRD_PERSON, CameraRigTypes.ViewMode3D.FIRST_PERSON]
	)
	rig.set_zoom_normalized(0.15, true)
	assert_eq(rig.get_view_mode(), CameraRigTypes.ViewMode3D.FIRST_PERSON)
	rig.set_zoom_normalized(0.25, true)
	assert_eq(rig.get_view_mode(), CameraRigTypes.ViewMode3D.THIRD_PERSON)
...
func test_metrics_are_projection_neutral_and_driver_disables_native_camera() -> void:
	...
	rig.set_output_driver(CameraRigTypes.OutputDriver.PHANTOM)
	assert_false(rig.get_camera().current)
	rig._process(1.0 / 60.0)
	assert_gt(bridge.third_person_calls, 0)
	assert_gt(bridge.projection_calls, 0)
	rig.set_output_driver(CameraRigTypes.OutputDriver.NATIVE)
...
func test_native_spring_arm_compresses_for_an_obstacle_and_recovers() -> void:
	var rig: CameraRig3D = await _create_rig()
	var local_preset: CameraPreset3D = rig.preset.duplicate(true) as CameraPreset3D
	local_preset.zoom_mechanism = CameraRigTypes.ZoomMechanism3D.DOLLY
	local_preset.min_distance = 2.0
	...
	local_preset.occlusion_mode = CameraRigTypes.OcclusionMode.PULL_IN
...
func test_active_occlusion_search_does_not_thrash_at_an_obstacle_edge() -> void:
	...
	local_preset.min_pitch_degrees = -1.0
	local_preset.max_pitch_degrees = 1.0
	local_preset.occlusion_mode = CameraRigTypes.OcclusionMode.PULL_IN_AND_SEARCH
	...
	rig._process(0.05)
	assert_lt(
		float(rig.get_view_metrics()[&"actual_distance"]),
		float(rig.get_view_metrics()[&"desired_distance"]),
		"The fixture must begin inside the SpringArm collision edge."
	)
...
func _create_rig() -> CameraRig3D:
	var rig: CameraRig3D = RIG_SCENE.instantiate() as CameraRig3D

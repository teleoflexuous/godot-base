...
const RIG_SCENE: PackedScene = preload("res://addons/camera_rigs/scenes/camera_rig_2d.tscn")
...
func test_scene_has_scene_first_camera_and_effect_pivots() -> void:
	var rig: CameraRig2D = RIG_SCENE.instantiate() as CameraRig2D
...
func test_positive_zoom_steps_zoom_in_and_pointer_anchor_stays_fixed() -> void:
	var rig: CameraRig2D = await _create_rig()
	var local_preset: CameraPreset2D = rig.preset.duplicate(true) as CameraPreset2D
	local_preset.bounds_enabled = false
	local_preset.zoom_enabled = true
	local_preset.zoom_anchor = CameraRigTypes.ZoomAnchor.POINTER_WORLD_HIT
...
func test_follow_interruption_policies_are_distinct() -> void:
	...
	local_preset.follow_interruption = CameraRigTypes.FollowInterruption.OFFSET_WHILE_FOLLOWING
	...
	local_preset.follow_interruption = CameraRigTypes.FollowInterruption.HARD_LOCK
	rig.recenter()
	rig.snap_to_target()
	...
	local_preset.follow_interruption = CameraRigTypes.FollowInterruption.BREAK_ON_PAN

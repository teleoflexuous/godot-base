...
func test_first_person_hide_mode_restores_visibility() -> void:
	...
	var visibility: CameraViewVisibility3D = CameraViewVisibility3D.new()
	visibility.visual_targets = [NodePath("../Body")]
	visibility.visibility_mode = CameraViewVisibility3D.VisibilityMode.HIDE
	visibility.transition_speed = 100.0
	root.add_child(visibility)
	add_child_autofree(root)

	visibility.apply_view_mode(CameraRigTypes.ViewMode3D.FIRST_PERSON)
	visibility._process(1.0)
	assert_false(visual.visible)
	visibility.apply_view_mode(CameraRigTypes.ViewMode3D.THIRD_PERSON)
	visibility._process(1.0)
...
func test_native_transparency_mode_restores_authored_value() -> void:
	...
	var visibility: CameraViewVisibility3D = CameraViewVisibility3D.new()
	visibility.visual_targets = [NodePath("../Body")]
	visibility.visibility_mode = CameraViewVisibility3D.VisibilityMode.NATIVE_TRANSPARENCY

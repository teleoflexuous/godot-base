class_name ProperCameraBaseSettingsControl
...
func _build_controls_if_needed() -> void:
	...
	if bundle_result != OK:
		push_warning("ProperCameraBaseSettingsControl could not connect its capability selector.")
	...
	if apply_result != OK:
		push_warning("ProperCameraBaseSettingsControl could not connect its apply button.")

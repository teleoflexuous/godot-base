class_name ProperCameraGuideRemappingControl
extends VBoxContainer

@export var controller: ProperCameraGuideRemappingController
...
func _resolve_replace() -> void:
	controller.resolve_collision(ProperCameraGuideRemappingController.CollisionResolution.REPLACE)
	controller.apply_configuration()


func _resolve_cancel() -> void:
	controller.resolve_collision(ProperCameraGuideRemappingController.CollisionResolution.CANCEL)


func _on_collision_custom_action(action: StringName) -> void:
	if action == &"swap":
		controller.resolve_collision(ProperCameraGuideRemappingController.CollisionResolution.SWAP)
		controller.apply_configuration()

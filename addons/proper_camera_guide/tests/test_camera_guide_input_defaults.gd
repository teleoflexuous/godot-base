...
func test_builder_creates_mergeable_device_contexts_and_typed_actions() -> void:
	var defaults: ProperCameraGuideDefaults = ProperCameraGuideDefaultsBuilder.build()
...
func test_context_set_selects_all_enabled_device_families() -> void:
	...
	var desktop_contexts: Array[GUIDEMappingContext] = defaults.context_set.contexts_for_mask(
		ProperCameraGuideContextSet.KEYBOARD | ProperCameraGuideContextSet.MOUSE
	)
	...
	assert_eq(defaults.context_set.contexts_for_mask(ProperCameraGuideContextSet.TOUCH).size(), 1)
...
func test_guide_adapter_gives_active_touch_pointer_ownership() -> void:
	...
	var adapter: ProperCameraGuideAdapter = ProperCameraGuideAdapter.new()
